...
article](https://pharmaverse.github.io/rtables/latest-tag/articles/dev-guide/dg_split_machinery.html)
...
[`debugonce`](https://rdrr.io/r/base/debug.html)`(``build_table``)`\
\
...
[`is`](https://rdrr.io/r/methods/is.html)`(``lyt``, ``"PreDataTableLayouts"``)`\
\
`lyt`` ``|>`` `[`build_table`](https://pharmaverse.github.io/rtables/reference/build_table.md)`(``DM``)`
...
`lyt`` ``<-`` `[`fix_dyncuts`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``lyt``, ``df``)`` ``# Create the splits that depends on data`\
...
`df`` ``<-`` ``fix_split_vars``(``lyt``, ``df``, char_ok ``=`` `[`is.null`](https://rdrr.io/r/base/NULL.html)`(``col_counts``)``)`\
...
[here](https://pharmaverse.github.io/rtables/latest-tag/articles/dev-guide/dg_table_hierarchy.html).
...
\
`clayout`` ``<-`` `[`clayout`](https://pharmaverse.github.io/rtables/reference/col_accessors.md)`(``lyt``)`` ``# Extracts column split and info`\
\
`if`` ``(`[`is.null`](https://rdrr.io/r/base/NULL.html)`(``topleft``)``)`` ``{`\
`  ``topleft`` ``<-`` `[`top_left`](https://pharmaverse.github.io/rtables/reference/top_left.md)`(``lyt``)`` ``# If top_left is not present in build_table, it is taken from lyt`\
`}`\
\
`ctree`` ``<-`` `[`coltree`](https://pharmaverse.github.io/rtables/reference/col_accessors.md)`(``clayout``, df ``=`` ``df``, rtpos ``=`` ``rtpos``)`` ``# Main constructor of LayoutColTree`\
...
\
`colextras`` ``<-`` `[`col_extra_args`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``ctree``)`` ``# retrieves extra_args from the tree. It may not be used`
...
`  `[`analyze`](https://pharmaverse.github.io/rtables/reference/analyze.md)`(``"BMRKR1"``)`\
\
`rlyt`` ``<-`` ``rtables``:::`[`rlayout`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``lyt``)`\
[`str`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``rlyt``, max.level ``=`` ``2``)`
