...
`# Known Subclasses: "DataRow", "ContentRow", "LabelRow"`\
\
[`getClass`](https://rdrr.io/r/methods/getClass.html)`(``"VTableTree"``)`
...
`  `[`split_rows_by`](https://pharmaverse.github.io/rtables/reference/split_rows_by.md)`(``"ARM"``, section_div ``=`` ``"+"``)`` ``|>`\
...
`    afun ``=`` ``function``(``x``)`` `[`list`](https://rdrr.io/r/base/list.html)`(``"Mean"`` ``=`` `[`mean`](https://rdrr.io/r/base/mean.html)`(``x``)``, ``"Standard deviation"`` ``=`` `[`sd`](https://rdrr.io/r/stats/sd.html)`(``x``)``)``,`\
`    format ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``"Mean"`` ``=`` ``"xx."``, ``"Standard deviation"`` ``=`` ``"xx."``)``,`\
`    section_div ``=`` ``"~"`\
`  ``)`\
\
...
\
[`setMethod`](https://rdrr.io/r/methods/setMethod.html)`(``"section_div<-"``, ``"VTableTree"``, ``function``(``obj``, ``value``, ``only_sep_sections`` ``=`` ``FALSE``)`` ``{`\
`  ``char_v`` ``<-`` `[`as.character`](https://rdrr.io/r/base/character.html)`(``value``)`\
`  ``tree_depths`` ``<-`` `[`unname`](https://rdrr.io/r/base/unname.html)`(`[`vapply`](https://rdrr.io/r/base/lapply.html)`(`[`collect_leaves`](https://pharmaverse.github.io/rtables/reference/collect_leaves.md)`(``obj``)``, ``tt_level``, `[`numeric`](https://rdrr.io/r/base/numeric.html)`(``1``)``)``)`\
`  ``max_tree_depth`` ``<-`` `[`max`](https://rdrr.io/r/base/Extremes.html)`(``tree_depths``)`\
`  `[`stopifnot`](https://rdrr.io/r/base/stopifnot.html)`(`[`is.logical`](https://rdrr.io/r/base/logical.html)`(``only_sep_sections``)``)`\
`  ``.check_char_vector_for_section_div``(``char_v``, ``max_tree_depth``, `[`nrow`](https://rdrr.io/r/base/nrow.html)`(``obj``)``)`\
\
`  ``# Automatic establishment of intent`\
`  ``if`` ``(`[`length`](https://rdrr.io/r/base/length.html)`(``char_v``)`` ``<`` `[`nrow`](https://rdrr.io/r/base/nrow.html)`(``obj``)``)`` ``{`\
`    ``only_sep_sections`` ``<-`` ``TRUE`\
`  ``}`\
\
`  ``# Case where only separators or splits need to change externally`\
`  ``if`` ``(``only_sep_sections`` ``&&`` `[`length`](https://rdrr.io/r/base/length.html)`(``char_v``)`` ``<`` `[`nrow`](https://rdrr.io/r/base/nrow.html)`(``obj``)``)`` ``{`\
`    ``if`` ``(`[`length`](https://rdrr.io/r/base/length.html)`(``char_v``)`` ``==`` ``1``)`` ``{`\
`      ``char_v`` ``<-`` `[`rep`](https://rdrr.io/r/base/rep.html)`(``char_v``, ``max_tree_depth`` ``-`` ``1``)`` ``# -1 is the data row`\
`    ``}`\
`    ``# Case where char_v is longer than the max depth`\
`    ``char_v`` ``<-`` ``char_v``[`[`seq_len`](https://rdrr.io/r/base/seq.html)`(`[`min`](https://rdrr.io/r/base/Extremes.html)`(``max_tree_depth``, `[`length`](https://rdrr.io/r/base/length.html)`(``char_v``)``)``)``]`\
`    ``# Filling up with NAs the rest of the tree depth section div chr vector`\
`    ``missing_char_v_len`` ``<-`` ``max_tree_depth`` ``-`` `[`length`](https://rdrr.io/r/base/length.html)`(``char_v``)`\
`    ``char_v`` ``<-`` `[`c`](https://rdrr.io/r/base/c.html)`(``char_v``, `[`rep`](https://rdrr.io/r/base/rep.html)`(``NA_character_``, ``missing_char_v_len``)``)`\
`    ``# char_v <- unlist(`\
...
`  ``content_row_tbl`` ``<-`` `[`content_table`](https://pharmaverse.github.io/rtables/reference/content_table.md)`(``obj``)`\
`  ``is_content_table`` ``<-`` `[`isS4`](https://rdrr.io/r/base/isS4.html)`(``content_row_tbl``)`` ``&&`` `[`nrow`](https://rdrr.io/r/base/nrow.html)`(``content_row_tbl``)`` ``>`` ``0`\
\
`  ``# Main table structure change`\
`  ``if`` ``(`[`labelrow_visible`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``obj``)`` ``||`` ``is_content_table``)`` ``{`\
`    ``if`` ``(``only_sep_sections``)`` ``{`\
`      ``# Only tables are modified`\
`      ``trailing_section_div``(`[`tt_labelrow`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``obj``)``)`` ``<-`` ``NA_character_`\
`      ``trailing_section_div``(``obj``)`` ``<-`` ``char_v``[``1``]`\
`      `[`section_div`](https://pharmaverse.github.io/rtables/reference/section_div.md)`(`[`tree_children`](https://pharmaverse.github.io/rtables/reference/tree_children.md)`(``obj``)``, only_sep_sections ``=`` ``only_sep_sections``)`` ``<-`` ``char_v``[``-``1``]`\
`    ``}`` ``else`` ``{`\
`      ``# All leaves are modified`\
`      ``trailing_section_div``(`[`tt_labelrow`](https://pharmaverse.github.io/rtables/reference/int_methods.md)`(``obj``)``)`` ``<-`` ``char_v``[``1``]`\
`      ``trailing_section_div``(``obj``)`` ``<-`` ``NA_character_`\
`      `[`section_div`](https://pharmaverse.github.io/rtables/reference/section_div.md)`(`[`tree_children`](https://pharmaverse.github.io/rtables/reference/tree_children.md)`(``obj``)``, only_sep_sections ``=`` ``only_sep_sections``)`` ``<-`` ``char_v``[``-``1``]`\
`    ``}`\
`  ``}`` ``else`` ``{`\
`    `[`section_div`](https://pharmaverse.github.io/rtables/reference/section_div.md)`(`[`tree_children`](https://pharmaverse.github.io/rtables/reference/tree_children.md)`(``obj``)``, only_sep_sections ``=`` ``only_sep_sections``)`` ``<-`` ``char_v`\
`  ``}`\
`  ``obj`\
`}``)`
...
[`formatters::toString`](https://pharmaverse.github.io/formatters/latest-tag/reference/tostring.html).
